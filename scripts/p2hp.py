...
class P2HP(UIWrapper):
    ...
    def ui(self, is_img2img) -> list:
        with gr.Accordion('Prompt Optimization', open=True):
            ...
            lr = gr.Slider(value=10e-02, default=10e-02, maximum=0.2, minimum=1e-03, step = 0.001, label='p2hp_lr')
            ...
            loss_weight = gr.Slider(value=1.0, default=0.5, step=0.01, minimum=0, maximum=1, label='p2hp_loss_weight')
            loss_tt = gr.Slider(value=1.0, default=0.5, step=0.01, minimum=0, maximum=1, label='p2hp_loss_tt')
            loss_spar = gr.Slider(value=1.0, default=0.1, step=0.01, minimum=0, maximum=1, label='p2hp_loss_sparsity')
            loss_ti = gr.Slider(value=1.0, default=0.5, step=0.01, minimum=0, maximum=1, label='p2hp_loss_ti')
